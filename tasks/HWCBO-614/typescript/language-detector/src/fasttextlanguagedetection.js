...
// 使用 require 替代 import，确保兼容性
var FastTextLanguageDetection = require('@smodin/fast-text-language-detection');
// 初始化检测器
var detector = new FastTextLanguageDetection();
// 语言检测函数
function detectLanguage(text) {
    return __awaiter(this, void 0, void 0, function () {
        var results, topResult, language, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    _a.trys.push([0, 4, , 5]);
                    if (!!detector.initialized) return [3 /*break*/, 2];
                    return [4 /*yield*/, detector.init()];
                case 1:
                    _a.sent();
                    detector.initialized = true; // 添加标志
                    _a.label = 2;
                case 2: return [4 /*yield*/, detector.predict(text)];
                case 3:
                    results = _a.sent();
                    topResult = results[0];
                    language = topResult.lang === 'fr' ? 'fr' : 'en';
                    return [2 /*return*/, { language: language, confidence: topResult.prob }];
                case 4:
                    error_1 = _a.sent();
                    console.error('FastText detection failed:', error_1);
                    return [2 /*return*/, { language: 'en', confidence: 0 }]; // 默认值
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, detectLanguage(testCase.text)];
                case 2:
                    result = _a.sent();
                    ...
                    console.log("Expected: ".concat(testCase.expectedLanguage));
                    console.log("Detected: ".concat(result.language, ", Confidence: ").concat(result.confidence.toFixed(4), ", Result: ").concat(isCorrect ? "Correct" : "Incorrect"));
                    console.log("---");
                    _a.label = 3;
                case 3:
                    _i++;
